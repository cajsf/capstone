...
SAVE_ROOT = r"C:\Users\hyi8402\Desktop\Capstone\Code\3.result\tgcn_tsam_singleview_window"

USE_LABEL_KEY = "primary_label"
MIN_VALID_FRAMES = 4

# ---- 논문스럽게 바꾼 핵심: window 기반 ----
WINDOW_SIZE = 48
WINDOW_STRIDE = 24
MAX_WINDOWS_PER_CLIP = None   # None이면 전부 사용, 너무 많으면 예: 8
...
def sliding_window_indices(length, window_size, stride):
    """
    길이 length인 시퀀스를 여러 개의 window index로 분할.
    length < window_size면 center/pad 1개 반환.
    """
    if length <= 0:
...
def preprocess_sample_to_windows(
    ...
    """
    sample 1개(clip 1개)를 여러 window sample로 변환
    반환: list of dict
    """
    ...
    for win_idx, idxs in enumerate(window_indices):
        kp_win = keypoints[idxs]
        conf_win = conf[idxs]

        if use_conf_channel:
            x = np.concatenate([kp_win, conf_win[..., None]], axis=-1)  # (T,V,3)
        else:
            x = kp_win                                                  # (T,V,2)
...
class TGCBlock(nn.Module):
    """
    single-view TGC block
    GConv -> BN -> GELU -> TConv -> BN -> Dropout + Residual
    """
...
class TSAM(nn.Module):
    """
    입력 feature (N, C, T, V)를 (N, T*V, C)로 펼쳐
    self-attention으로 시공간 토큰 관계를 학습
    """
    ...
    def forward(self, x):
        ...
        tokens = x.permute(0, 2, 3, 1).contiguous().view(N, T * V, C)  # (N, L, C)
...
class TGCN_TSAM_Classifier(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.mean(dim=-1).mean(dim=-1)  # global avg pool over V and T
        x = self.head(x)
        return x
...
def aggregate_clip_level_results(targets, probs, clip_dirs):
    """
    window-level 결과를 clip-level 평균 확률로 집계
    """
    clip_prob_sum = defaultdict(list)
    clip_target = {}

    for y, p, c in zip(targets, probs, clip_dirs):
        clip_prob_sum[c].append(p)
        if c in clip_target:
            if clip_target[c] != y:
    ...
    for c in clip_prob_sum.keys():
        mean_prob = np.mean(np.stack(clip_prob_sum[c], axis=0), axis=0)
...
def main():
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        # early stopping / scheduler 기준을 clip-level로
